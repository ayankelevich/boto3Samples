...
# both methods work:
print('Account Id:', response['Account'], response.get('Account'))
print(response['UserId'], response['Arn'])

my_own_account_id = response.get('Account')
...
for each_snap in ec2_con_re.snapshots.filter(OwnerIds=[my_own_account_id]):
